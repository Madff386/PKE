...
if task == 'encrypt':
    ...
    enout = StringVar()
    labelen = ttk.Entry(outen, textvariable="enout")
    labelen.grid(column=0, row=0, sticky=(W, E),  padx=10, pady=10)
    encrypted = str(encrypted)
    encrypted = encrypted.replace(',', '')
    encrypted = encrypted.replace(']', '')
    encrypted = encrypted.replace('[', '')
    labelen.insert(0, '%s' % encrypted)
    labelen.state(['readonly'])
    ...
    deout = StringVar()
    labelde = ttk.Entry(outde, textvariable="deout")
    labelde.grid(column=0, row=0, sticky=(W, E),  padx=10, pady=10)
    decrypted = str(decrypted)
    decrypted = decrypted.replace(',', '')
    decrypted = decrypted.replace(']', '')
    decrypted = decrypted.replace('[', '')
    labelde.insert(0, '%s' % decrypted)
    labelde.state(['readonly'])
